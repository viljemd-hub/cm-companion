...
It is an open-source reference client: everything it does, it does through CM's public **[Bridge Protocol v1](docs/bridge_protocol_v1.md)**, the same API surface any third-party integrator would use. There is no private shortcut into CM's internals anywhere in this app. If a screen needs a capability the Bridge doesn't expose, that's a documented Bridge scope added on the server side — never a special case here. Building your own client instead? [`docs/bridge_protocol_v1.md`](docs/bridge_protocol_v1.md) is the full reference — pairing, auth, every endpoint, every scope.
